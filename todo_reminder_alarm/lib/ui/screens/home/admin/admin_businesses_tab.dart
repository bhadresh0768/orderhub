        ...
        return ordersAsync.when(
          data: (orders) {
            return usersAsync.when(
              data: (users) {
                final ownerNamesById = <String, String>{
                ...
                final filtered = businesses.where((b) {
                  final ownerName = ((b.ownerName ?? '').trim().isNotEmpty
                          ? b.ownerName!
                          : (ownerNamesById[b.ownerId] ?? ''))
                      .toLowerCase();
                ...
                return ListView(
                  ...
                  children: [
                      ...
                      children: [
                        Expanded(
                          child: TextField(
                            ...
                            onChanged: (value) {
                              ref
                                  .read(adminSearchProvider(_searchKey).notifier)
                                  .state = value;
                            },
                          ),
                        ),
                    ...
                    ...filtered.map((business) {
                      ...
                      final ownerName = (business.ownerName ?? '').trim().isNotEmpty
                          ? business.ownerName!.trim()
                          : (ownerNamesById[business.ownerId] ?? '').trim();
                      final now = DateTime.now();
